...
- (void)loadView {
    CGRect viewBounds = CGRectMake(0,0,320,480);
...
- (void)SetColor
{
    if([[[DrawingIpadAppDelegate getGlobalInfo] valueForKey:@"BrushOption"] isEqualToString:@"Solid"])
    {
        self.selectedColor = [ColorPickerController colorFromHexValue:[[DrawingIpadAppDelegate getGlobalInfo] valueForKey:@"Haxcode"]];
        [self moveSelectors];
    }
}
- (void)viewDidUnload {
...
- (void)setColorValues {
    if (_colorView) {
        ...
        [[DrawingIpadAppDelegate getGlobalInfo] setValue:[NSString stringWithFormat:@"%f",_hsvColor.hue] forKey:@"hue"];
        [[DrawingIpadAppDelegate getGlobalInfo] setValue:[NSString stringWithFormat:@"%f",_hsvColor.saturation] forKey:@"saturation"];
        [[DrawingIpadAppDelegate getGlobalInfo] setValue:[NSString stringWithFormat:@"%f",_hsvColor.brightness] forKey:@"brightness"];
        
        NSLog(@"main:-->%@",[[DrawingIpadAppDelegate getGlobalInfo] description]);
